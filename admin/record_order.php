...
try {
    ...
    $sql = "INSERT INTO orders (order_type, customer_type, total_amount, amount_paid, change_amount, order_date) 
            VALUES ('$orderType', '$customerType', $totalAmount, $amountPaid, $changeAmount, NOW())";
...
$stmt = $conn->prepare("INSERT INTO sales ( saleDate, product_name, quantity, price, total, orderType, customerType) VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        foreach ($sales as $sale) {
            ...
            $stmt->bind_param('isidd', $productId, $product_name, $quantity, $price, $total, $orderType, $customerType);
            $stmt->execute();
        }
