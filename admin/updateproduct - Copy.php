...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['product_id'], $_POST['editProductName'], $_POST['editCategory'], $_POST['editStock'], $_POST['editPrice'], $_POST['editStatus'], $_POST['editDate'], $_POST['editExpirationDate'])) {
    ...
    $unit = htmlspecialchars($_POST['editUnit']);
    ...
    $query = "UPDATE products SET product_name=?, unit=?, category=?, stock=?, price=?, status=?, date=?, expiration_date=? WHERE product_id=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ssiidssii", $product_name, $unit, $category, $stock, $price, $status, $date, $expiration_date, $product_id);
    $stmt->execute();
